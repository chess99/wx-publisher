...
function renderManifesto(module: AdvancedModule, p: AdvancedPalette): string {
  const f = module.fields
  return `<section data-mpa-action-id="manifesto" style="margin:0 0 32px;"><section style="padding:18px;background:linear-gradient(180deg, ${p.accentSoft} 0%, ${p.surface} 100%);border:1px solid ${p.border};border-radius:16px;box-shadow:${p.shadow};">${label(f.label ?? "", p, `margin:0 0 10px;background:${p.surface};border:1px solid ${p.border};`)}<p style="margin:0;font-size:20px;font-weight:950;color:${p.text};line-height:1.35;">${esc(f.title)}</p>${paragraph(f.body ?? "", p, "margin:10px 0 0;")}<section style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin:16px 0 0;">${manifestoList("相信", splitList(f.believe), true, p)}${manifestoList("反对", splitList(f.against), false, p)}</section>${f.note ? `<p style="margin:14px 0 0;padding-top:12px;border-top:1px solid ${p.mutedBorder};font-size:13px;color:${p.muted};line-height:1.55;">${esc(f.note)}</p>` : ""}</section></section>`
}

function renderMythFact(module: AdvancedModule, p: AdvancedPalette): string {
  return `<section data-mpa-action-id="myth-fact" style="margin:0 0 30px;">${sectionTitle(module.title, p)}${module.rows.map(row => `<section style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;margin:0 0 12px;"><section style="padding:13px 14px;border-radius:12px;background:${p.surface};border:1px solid ${p.mutedBorder};box-shadow:${p.shadow};"><p style="display:inline-block;margin:0 0 7px;padding:3px 8px;border-radius:999px;background:${p.surfaceAlt};border:1px solid ${p.mutedBorder};color:${p.muted};font-size:13px;font-weight:900;line-height:1.4;">误区</p><p style="margin:0;font-size:15px;font-weight:750;color:${p.text};line-height:1.68;">${esc(row[0])}</p></section><section style="padding:13px 14px;border-radius:12px;background:${p.accentSoft};border:1px solid ${p.border};box-shadow:${p.shadow};"><p style="display:inline-block;margin:0 0 7px;padding:3px 8px;border-radius:999px;background:${p.surface};border:1px solid ${p.border};color:${p.accentDark};font-size:13px;font-weight:900;line-height:1.4;">事实</p><p style="margin:0;font-size:15px;font-weight:750;color:${p.text};line-height:1.68;">${esc(row[1])}</p></section></section>`).join("")}</section>`
}
