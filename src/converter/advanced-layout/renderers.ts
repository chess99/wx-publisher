...
function renderSteps(module: AdvancedModule, p: AdvancedPalette): string {
  return `<section data-mpa-action-id="steps" style="margin:0 0 32px;">${sectionTitle(module.title, p)}${module.rows.map((row, rowIndex) => {
    const [stepIndex, title, body, note] = row
    const connector = rowIndex === module.rows.length - 1 ? "" : `<span style="width:2px;flex:1 1 auto;background:linear-gradient(${p.accentSofter}, transparent);margin-top:8px;border-radius:999px;"></span>`
    return `<section style="display:flex;gap:12px;align-items:stretch;margin-bottom:12px;"><section style="display:flex;flex-direction:column;align-items:center;flex-shrink:0;"><span style="width:34px;height:34px;border-radius:10px;display:flex;align-items:center;justify-content:center;background:${p.surface};color:${p.accentDark};font-size:13px;font-weight:800;border:1px solid ${p.border};box-shadow:${p.shadow};">${esc(stepIndex)}</span>${connector}</section><section style="flex:1 1 0%;background:linear-gradient(180deg, ${p.surface} 0%, ${p.surfaceAlt} 100%);border:1px solid ${p.mutedBorder};border-radius:12px;padding:14px 16px;box-shadow:${p.shadow};"><p style="font-size:17px;font-weight:800;color:${p.text};margin:0 0 6px;">${esc(title)}</p>${paragraph(body ?? "", p, `color:${p.text};`)}${note ? `<p style="margin:6px 0 0;font-size:13px;color:${p.muted};line-height:1.55;">${esc(note)}</p>` : ""}</section></section>`
  }).join("")}</section>`
}
...
function renderTimeline(module: AdvancedModule, p: AdvancedPalette): string {
  return `<section data-mpa-action-id="timeline" style="margin:0 0 30px;">${sectionTitle(module.title, p)}${module.rows.map((row, index) => {
    const [time, title, body] = row
    const connector = index === module.rows.length - 1 ? "" : `<span style="flex:1;width:2px;background:${p.mutedBorder};margin-top:4px;min-height:20px;"></span>`
    return `<section style="display:flex;gap:12px;align-items:flex-start;padding:0 0 16px;min-height:48px;"><section style="display:flex;flex-direction:column;align-items:center;flex-shrink:0;padding-top:3px;"><span style="width:10px;height:10px;border-radius:999px;background:${p.accentDark};flex-shrink:0;box-shadow:0 0 0 3px ${p.accentSoft};"></span>${connector}</section><section style="flex:1;min-width:0;padding-bottom:4px;"><span style="display:inline-block;font-size:13px;font-weight:700;color:${p.accentDark};background:${p.accentSoft};padding:2px 8px;border-radius:999px;margin:0 0 4px;line-height:1.5;">${esc(time)}</span><p style="font-size:17px;font-weight:700;color:${p.text};margin:0 0 2px;line-height:1.5;">${esc(title)}</p>${paragraph(body ?? "", p, `color:${p.text};line-height:1.65;`)}</section></section>`
  }).join("")}</section>`
}
...
function renderInfographic(module: AdvancedModule, p: AdvancedPalette): string {
  ...
  return `<section data-mpa-action-id="infographic" style="margin:0 0 32px;"><section style="position:relative;overflow:hidden;border-radius:22px;background:linear-gradient(135deg, ${p.surface} 0%, ${p.surfaceAlt} 100%);border:1px solid ${p.mutedBorder};box-shadow:0 8px 24px rgba(85,85,85,0.08);color:${p.text};"><section style="padding:24px 22px 20px;display:flex;flex-direction:column;justify-content:space-between;min-height:238px;" data-infographic-type="${attr(f.type ?? "statement")}">
<section style="display:flex;align-items:center;justify-content:flex-start;gap:8px;margin-bottom:20px;"><section style="width:30px;height:30px;border-radius:999px;border:1px solid ${p.mutedBorder};background:${p.surface};display:inline-flex;align-items:center;justify-content:center;gap:4px;flex-shrink:0;" aria-hidden="true"><span style="display:inline-block;width:5px;height:5px;border-radius:999px;opacity:0.9;background:${p.accent};"></span><span style="display:inline-block;width:10px;height:2px;border-radius:999px;opacity:0.48;background:${p.accent};"></span></section>${label(f.eyebrow ?? "", p, `text-transform:none;color:${p.accent};display:inline-flex;align-items:center;border:1px solid ${p.border};background:${p.accentSoft};`)}</section>
<section style="max-width:100%;"><section style="display:flex;flex-direction:column;"><span style="display:inline-block;width:54px;height:2px;border-radius:999px;background:${p.accentDark};margin-bottom:16px;"></span>
...
</section>${f.quote ? `<p style="margin:18px 0 0;padding:12px 0 0 14px;border-left:2px solid ${p.border};font-size:22px;font-weight:800;color:${p.text};line-height:1.62;">${esc(f.quote)}</p>` : ""}</section>
${f.note ? `<section style="display:flex;align-items:flex-end;justify-content:space-between;gap:12px;margin-top:22px;"><p style="margin:0;font-size:13px;color:${p.muted};line-height:1.55;letter-spacing:0.5px;">${esc(f.note)}</p></section>` : ""}
...
function renderBridge(module: AdvancedModule, p: AdvancedPalette): string {
  const f = module.fields
  return `<section data-mpa-action-id="bridge" style="display:flex;align-items:stretch;gap:12px;margin:8px 0 30px;"><section style="width:3px;border-radius:999px;background:linear-gradient(180deg, ${p.accentDark}, ${p.accentSofter});flex-shrink:0;"></section><section style="flex:1 1 0%;min-width:0;padding:14px 15px;background:${p.surface};border:1px solid ${p.mutedBorder};border-radius:12px;box-shadow:${p.shadow};">${label(f.label ?? "继续阅读", p, `margin:0 0 7px;border:1px solid ${p.border};`)}<p style="margin:0;font-size:17px;font-weight:900;color:${p.text};line-height:1.45;">${esc(f.title)}</p>${paragraph(f.body ?? "", p, "margin:8px 0 0;")}${f.next ? `<p style="margin:12px 0 0;padding:10px 12px;border-radius:10px;background:${p.accentSoft};border:1px solid ${p.border};color:${p.accentDark};font-size:14px;font-weight:850;line-height:1.55;">${esc(f.next)}</p>` : ""}</section></section>`
}

function renderManifesto(module: AdvancedModule, p: AdvancedPalette): string {
  const f = module.fields
  return `<section data-mpa-action-id="manifesto" style="margin:0 0 30px;padding:20px 18px 16px;background:${p.surface};border:1px solid ${p.mutedBorder};border-radius:16px;box-shadow:${p.shadow};">${label(f.label ?? "", p, `margin:0 0 10px;padding:4px 10px;border:1px solid ${p.border};font-weight:900;line-height:1.42;letter-spacing:0.7px;`)}<p style="margin:0;font-size:17px;font-weight:900;color:${p.text};line-height:1.42;">${esc(f.title)}</p>${paragraph(f.body ?? "", p, "margin:9px 0 0;line-height:1.74;")}${manifestoList("我相信", splitList(f.believe), true, p)}${manifestoList("我拒绝", splitList(f.against), false, p)}${f.note ? `<p style="margin:14px 0 0;padding-top:12px;border-top:1px solid ${p.mutedBorder};font-size:13px;color:${p.muted};line-height:1.55;">${esc(f.note)}</p>` : ""}</section>`
}

function renderMythFact(module: AdvancedModule, p: AdvancedPalette): string {
  return `<section data-mpa-action-id="myth-fact" style="margin:0 0 30px;">${sectionTitle(module.title, p)}${module.rows.map(row => `<section style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;margin:0 0 12px;"><section style="padding:13px 14px;border-radius:12px;background:${p.surface};border:1px solid ${p.mutedBorder};box-shadow:${p.shadow};"><p style="display:inline-block;margin:0 0 7px;padding:3px 8px;border-radius:999px;background:${p.surfaceAlt};border:1px solid ${p.mutedBorder};color:${p.muted};font-size:13px;font-weight:900;line-height:1.4;">误区</p><p style="margin:0;font-size:15px;font-weight:750;color:${p.text};line-height:1.68;">${esc(row[0])}</p></section><section style="padding:13px 14px;border-radius:12px;background:linear-gradient(135deg, ${p.accentSoft} 0%, ${p.surface} 70%, ${p.surfaceAlt} 100%);border:1px solid ${p.border};box-shadow:${p.shadow};"><p style="display:inline-block;margin:0 0 7px;padding:3px 8px;border-radius:999px;background:${p.accentSoft};border:1px solid ${p.border};color:${p.accentDark};font-size:13px;font-weight:900;line-height:1.4;">事实</p><p style="margin:0;font-size:15px;font-weight:800;color:${p.text};line-height:1.68;">${esc(row[1])}</p></section></section>`).join("")}</section>`
}
...
function renderChangelog(module: AdvancedModule, p: AdvancedPalette): string {
  ...
  const version = stringValue(data.version) || "v1"
  const date = stringValue(data.date)
  return `<section data-mpa-action-id="changelog" style="${cardStyle(p)}padding:16px;margin:24px 0;display:flex;flex-direction:column;gap:12px;"><div style="display:flex;justify-content:space-between;align-items:baseline;padding-bottom:8px;border-bottom:1px solid ${p.mutedBorder};"><span style="font-size:17px;font-weight:800;color:${p.text};font-variant-numeric:tabular-nums;">${esc(version)}</span>${date ? `<span style="font-size:13px;color:${p.muted};">${esc(date)}</span>` : ""}</div><div style="display:flex;flex-direction:column;gap:4px;">${groups.map(row => `<div style="font-size:15px;line-height:1.7;color:${p.textStrong};padding-left:12px;position:relative;"><span style="display:inline-block;padding:1px 8px;border-radius:8px;font-size:13px;font-weight:700;background:${p.accentSoft};color:${p.accentDark};margin-right:6px;">${esc(row[0])}</span>${esc(row[1])}</div>`).join("")}</div></section>`
}
...
function renderComparisonTable(module: AdvancedModule, p: AdvancedPalette): string {
  ...
  return `<section data-mpa-action-id="comparison-table" style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:8px;margin:24px 0;">${comparisonPanel(stringValue(left.title) || "方案 A", leftItems, true, p)}${comparisonPanel(stringValue(right.title) || "方案 B", rightItems, false, p)}</section>`
}

function renderDefinition(module: AdvancedModule, p: AdvancedPalette): string {
  const data = parseJsonObject(module.body)
  return `<section data-mpa-action-id="definition" style="${cardStyle(p)}padding:16px;margin:20px 0;display:flex;gap:12px;"><div style="flex:0 0 30%;min-width:96px;"><div style="display:inline-block;padding:4px 8px;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:${p.accentDark};background:${p.accentSoft};border-radius:999px;margin-bottom:4px;">${esc(stringValue(data.termLabel) || "定义")}</div><div style="font-size:17px;font-weight:700;color:${p.accentDark};line-height:1.4;">${esc(stringValue(data.term))}</div></div><div style="flex:1;border-left:1px solid ${p.mutedBorder};padding-left:12px;"><div style="font-size:15px;line-height:1.7;color:${p.textStrong};">${esc(stringValue(data.def))}</div></div></section>`
}
...
function renderQuestion(module: AdvancedModule, p: AdvancedPalette): string {
  ...
  return `<section data-mpa-action-id="question" style="margin:24px 0;display:flex;flex-direction:column;gap:12px;">${items.map(item => `<div data-mpa-action-id="question-item" style="${cardStyle(p)}padding:12px 16px;"><div style="display:flex;gap:8px;align-items:flex-start;font-size:15px;font-weight:700;color:${p.text};"><span style="flex-shrink:0;width:22px;height:22px;border-radius:999px;background:${p.accentDark};color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;">Q</span><span>${esc(item.q)}</span></div><div style="display:flex;gap:8px;align-items:flex-start;font-size:15px;color:${p.textStrong};line-height:1.7;margin-top:8px;"><span style="flex-shrink:0;width:22px;height:22px;border-radius:999px;background:${p.surfaceAlt};color:${p.accentDark};border:1px solid ${p.border};display:inline-flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;">A</span><span>${esc(item.a)}</span></div></div>`).join("")}</section>`
}
...
function renderQuoteCard(module: AdvancedModule, p: AdvancedPalette): string {
  ...
  return `<section data-mpa-action-id="quote-card" style="${cardStyle(p)}padding:24px 20px;margin:32px 0;text-align:center;background:linear-gradient(180deg, ${p.surface} 0%, ${p.accentSoft} 100%);"><div style="font-size:22px;line-height:1;color:${p.accentDark};opacity:0.32;font-family:Georgia, 'Times New Roman', serif;" aria-hidden="true">“</div><p style="font-size:22px;line-height:1.5;color:${p.text};font-weight:600;margin:12px 0;">${esc(stringValue(data.text))}</p>${attribution ? `<div style="display:inline-block;padding:4px 8px;font-size:12px;font-weight:700;letter-spacing:0.08em;text-transform:uppercase;color:${p.accentDark};background:${p.accentSoft};border-radius:999px;margin-top:12px;">${esc(attribution)}</div>` : ""}</section>`
}

function renderResourceList(module: AdvancedModule, p: AdvancedPalette): string {
  const items = parseJsonArray(module.body)
  return `<section data-mpa-action-id="resource-list" style="${cardStyle(p)}padding:16px;margin:24px 0;">${items.map(item => {
    const url = stringValue(item.url)
    return `<div data-mpa-action-id="resource-list-item" style="display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid ${p.mutedBorder};"><span style="flex-shrink:0;width:36px;height:36px;border-radius:8px;background:${p.accentSoft};color:${p.accentDark};display:inline-flex;align-items:center;justify-content:center;font-weight:700;">${esc(stringValue(item.icon) || "资源")}</span><div style="flex:1;min-width:0;"><div style="font-size:15px;font-weight:700;color:${p.text};line-height:1.4;">${esc(stringValue(item.name))}</div><div style="font-size:13px;color:${p.muted};line-height:1.5;">${esc(stringValue(item.desc))}</div></div>${url ? `<a href="${attr(safeUrl(url))}" style="font-size:13px;color:${p.accentDark};text-decoration:none;border-bottom:1px solid ${p.border};">查看</a>` : ""}</div>`
  }).join("")}</section>`
}

function renderStatRow(module: AdvancedModule, p: AdvancedPalette): string {
  const items = parseJsonArray(module.body)
  return `<section data-mpa-action-id="stat-row" style="${cardStyle(p)}display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:1px;background:${p.mutedBorder};padding:1px;margin:24px 0;overflow:hidden;">${items.map(item => `<div data-mpa-action-id="stat-row-cell" style="background:${p.surface};padding:16px 12px;text-align:center;display:flex;flex-direction:column;gap:4px;"><span style="font-size:26px;line-height:1;font-weight:800;color:${p.accentDark};font-variant-numeric:tabular-nums;">${esc(`${stringValue(item.value)}${stringValue(item.unit)}`)}</span><span style="font-size:13px;color:${p.muted};">${esc(stringValue(item.label))}</span>${stringValue(item.note) ? `<span style="font-size:12px;color:${p.muted};line-height:1.4;">${esc(stringValue(item.note))}</span>` : ""}</div>`).join("")}</section>`
}
...
function renderTweet(module: AdvancedModule, p: AdvancedPalette): string {
  ...
  return `<section data-mpa-action-id="tweet" style="${cardStyle(p)}padding:16px;margin:20px 0;display:flex;flex-direction:column;gap:8px;"><div style="display:flex;align-items:center;gap:8px;">${avatar ? `<span style="width:40px;height:40px;border-radius:999px;overflow:hidden;background:${p.accentSoft};border:1px solid ${p.border};display:inline-block;">${imageTag(avatar, name, "width:100% !important;height:100% !important;object-fit:cover;display:block;")}</span>` : `<span style="width:40px;height:40px;border-radius:999px;background:${p.accentSoft};color:${p.accentDark};display:inline-flex;align-items:center;justify-content:center;font-weight:700;">${esc(name.slice(0, 1))}</span>`}<div><div style="font-size:15px;font-weight:700;color:${p.text};line-height:1.3;">${esc(name)}</div>${meta ? `<div style="font-size:13px;color:${p.muted};line-height:1.3;">${esc(meta)}</div>` : ""}</div></div><div style="font-size:15px;line-height:1.7;color:${p.textStrong};white-space:pre-wrap;">${esc(stringValue(data.text))}</div>${stringValue(data.url) ? `<div style="font-size:13px;color:${p.muted};line-height:1.4;">${esc(stringValue(data.url))}</div>` : ""}</section>`
}

function renderGallery(module: AdvancedModule, p: AdvancedPalette): string {
  const images = parseMarkdownImages(module.body)
  const cards = images.map(img => `<section style="padding:16px 12px 12px 12px;border-radius:12px;background:${p.surface};border:1px solid ${p.mutedBorder};box-shadow:${p.shadow};flex:0 0 78%;max-width:360px;scroll-snap-align:start;scroll-snap-stop:always;display:flex;flex-direction:column;gap:8px;box-sizing:border-box;"><section style="border-radius:12px;overflow:hidden;background:${p.surface};aspect-ratio:4/3;width:100%;">${imageTag(img.src, img.alt, "display:block;width:100% !important;height:100% !important;object-fit:cover;")}</section>${img.alt ? `<p style="margin:0;font-size:13px;color:${p.muted};line-height:1.5;text-align:left;">${esc(img.alt)}</p>` : ""}</section>`).join("")
  const title = module.title ? `<p style="margin:8px 0 0;font-size:13px;color:${p.muted};text-align:center;line-height:1.5;">${esc(module.title)}</p>` : ""
  return `<section data-mpa-action-id="gallery" style="margin:32px 0;overflow:hidden;"><section style="display:flex;gap:12px;overflow-x:auto;scroll-snap-type:x mandatory;padding-bottom:8px;-webkit-overflow-scrolling:touch;">${cards}</section>${title}</section>`
}

function renderLongImage(module: AdvancedModule, p: AdvancedPalette): string {
  const image = parseMarkdownImages(module.body)[0]
  const title = module.title ? `<p style="margin:12px 0 0;padding:10px 16px;font-size:12px;color:${p.accentDark};text-align:center;line-height:1.5;font-weight:500;letter-spacing:0.3px;background:linear-gradient(135deg, ${p.surface} 0%, ${p.accentSoft} 100%);border:1px solid ${p.border};border-radius:12px;box-shadow:${p.shadow};">${esc(module.title)}</p>` : ""
  return `<section data-mpa-action-id="longimage" style="margin:32px 0;"><section style="margin:16px 8px 20px;padding:16px;background:linear-gradient(135deg, ${p.accentSoft}, ${p.surface});border:1px solid ${p.border};border-radius:12px;box-shadow:${p.accentShadow};position:relative;overflow:hidden;box-sizing:border-box;"><section class="longimage-scroll-container" style="max-height:420px;max-height:min(75vh,600px);overflow-y:auto;border-radius:8px;background:${p.surface};box-shadow:${p.shadow};border:1px solid ${p.mutedBorder};padding:4px;scrollbar-width:thin;scrollbar-color:${p.border} transparent;-webkit-overflow-scrolling:touch;box-sizing:border-box;">${image ? imageTag(image.src, image.alt, "width:100% !important;display:block;height:auto !important;") : ""}</section>${title}</section></section>`
}
...
function manifestoList(title: string, items: string[], accent: boolean, p: AdvancedPalette): string {
  const itemBackground = accent
    ? `linear-gradient(180deg, ${p.accentSoft} 0%, ${p.surfaceAlt} 100%)`
    : p.surfaceAlt
  const itemBorder = accent ? p.border : p.mutedBorder
  const bullet = accent ? p.accentDark : p.muted
  return `<section style="margin:15px 0 0;display:flex;flex-direction:column;gap:8px;"><p style="margin:0;font-size:13px;font-weight:900;color:${p.muted};line-height:1.45;letter-spacing:0.8px;">${title}</p>${items.map(item => `<section style="display:flex;align-items:flex-start;gap:8px;padding:10px 11px;border-radius:8px;background:${itemBackground};border:1px solid ${itemBorder};color:${p.text};font-size:15px;font-weight:750;line-height:1.66;"><span style="margin-top:0.62em;display:inline-block;width:6px;height:6px;border-radius:999px;background:${bullet};flex-shrink:0;"></span><p style="margin:0;">${esc(item)}</p></section>`).join("")}</section>`
}
...
function comparisonPanel(title: string, items: string[], positive: boolean, p: AdvancedPalette): string {
  const color = positive ? "#097249" : p.accentDark
  const symbol = positive ? "✓" : "→"
  return `<div style="${cardStyle(p)}padding:16px;border-top:3px solid ${color};"><div style="font-size:17px;font-weight:700;color:${p.text};margin-bottom:12px;display:flex;align-items:center;gap:8px;"><span style="width:18px;height:18px;border-radius:999px;background:${color};color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:12px;">${symbol}</span>${esc(title)}</div>${items.map(item => `<div style="padding:8px 0;border-bottom:1px solid ${p.mutedBorder};font-size:15px;line-height:1.6;color:${p.textStrong};">${symbol} ${esc(item)}</div>`).join("")}</div>`
}
